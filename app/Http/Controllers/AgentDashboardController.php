...
class AgentDashboardController extends Controller
{
    public function index()
    {
        ...
        $directBookings = Booking::where('agent_user_id', $agentId)
            ->with(['client', 'funeralHome', 'package'])
            ->get();

        $agentBookingIds = BookingAgent::where('agent_user_id', $agentId)
            ->pluck('booking_id');
        $indirectBookings = Booking::whereIn('id', $agentBookingIds)
            ->with(['client', 'funeralHome', 'package'])
            ->get();

        $allBookings = $directBookings->merge($indirectBookings)->unique('id');

        return view('agent.dashboard', ['bookings' => $allBookings]);
    }
...
public function updateInfo(Request $request, $bookingId)
{
    ...
    public function sendCustomizationRequest(Request $request, $bookingId)
    {
        ...
        try {
            ...
            if ($booking->client) {
                $booking->client->notify(new CustomizationRequestSubmitted($customized));
            }
